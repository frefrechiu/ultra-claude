...
status: phase-4-plan-01-complete-adapter-protocol-and-mixin-landed
last_updated: "2026-05-02T03:24:44Z"
progress:
  total_phases: 9
  completed_phases: 2
  total_plans: 7
...
**Current Focus:** Phase 4 plan 04-01 COMPLETE. Closed ADP-01, ADP-02, ADP-03, ADP-04, ADP-08 — `src/ultra_claude/exceptions.py` extended with `AdapterError` + `AdapterAuthError`, and `src/ultra_claude/adapters/__init__.py` + `src/ultra_claude/adapters/base.py` (`Adapter` `typing.Protocol` + `_SubprocessAdapterMixin._run_subprocess` enforcing the full subprocess invocation contract: stdin-piped prompt, UTF-8/replace, mandatory timeout, list-form argv, shell=False, empty-stdout defense for openai/codex#19945, cross-platform process-tree kill via POSIX killpg / Windows taskkill /T /F) landed via commits `e4423d0` + `eceb9da`. mypy --strict + ruff clean across all 3 files; full test suite 16/16 PASS (zero regression); 6 inline integration smoke checks covering FileNotFoundError, empty-stdout, auth-marker, happy-path, non-zero exit, TimeoutExpired-with-kill all PASS. Plans 04-02 (ClaudeAdapter) and 04-03 (tests + TST-05 lint) are now unblocked. Phase 5 (Stop Conditions) remains parallelizable. Phase 1 autonomous portion still complete; PKG-05 still pending user `twine upload` (independent of subsequent phases).

## Current Position

Phase: 04-adapter-protocol-claudeadapter — Plan 04-01 COMPLETE (1/3 plans, 5/7 ADP+TST requirements closed for the phase)
Next plan: 04-02 (ClaudeAdapter) — Wave 2, depends on 04-01
| Field | Value |
|-------|-------|
| Phase | 4 (Adapter Protocol & ClaudeAdapter) — IN PROGRESS (1/3 plans complete) |
| Plan | 04-01 COMPLETE; 04-02 (ClaudeAdapter) NEXT — Wave 2 |
| Status | ADP-01..04 + ADP-08 closed; `from ultra_claude.adapters import Adapter, _SubprocessAdapterMixin` works; `from ultra_claude.exceptions import AdapterError, AdapterAuthError` works; mypy --strict + ruff clean across all 3 files; 16/16 existing tests still PASS (zero regression); 6 integration smoke paths verified inline; LF-only on disk on all 3 files; ASCII-only source |
| Progress | 2/9 phases complete; 7/7 completed plans (Phase 1: 3/3, Phase 2: 2/2, Phase 3: 1/1, Phase 4: 1/3) |

```
[#######             ] 7/9 plans counted as complete on full-phase basis; Phase 4 plan 04-01 of 3 landed; 04-02 / 04-03 next
```
...
|--------|-------|
| Phases planned | 9 |
| Phases complete | 2 (Phases 2 + 3 fully closed; Phase 1 awaiting user twine upload to fully close PKG-05; Phase 4 in progress 1/3 plans) |
| Plans complete | 7 (Phase 1: 3/3, Phase 2: 2/2, Phase 3: 1/1, Phase 4: 1/3) |
| v1 requirements mapped | 58/58 |
| Requirements completed | 19 (PKG-02, PKG-03, PKG-04, PKG-07, CFG-01..05, TRX-01..05, ADP-01..04, ADP-08) |
...
### Phase 4 Progress (IN PROGRESS — 1/3 plans complete)
...
**Last action:** Executed Phase 4 plan 04-01 (Adapter Protocol + _SubprocessAdapterMixin). Extended `src/ultra_claude/exceptions.py` (now 3569 bytes, 80 lines, LF-only, ASCII-only) with `class AdapterError(Exception)` and `class AdapterAuthError(AdapterError)`; updated module docstring tense; `__all__` extended to `["ConfigError", "AdapterError", "AdapterAuthError"]` with `# noqa: RUF022` and a justifying comment about the intentional Phase-2-then-Phase-4 ordering. Created `src/ultra_claude/adapters/__init__.py` (923 bytes, 21 lines) re-exporting `Adapter` and `_SubprocessAdapterMixin`. Created `src/ultra_claude/adapters/base.py` (11805 bytes, 269 lines, LF-only, ASCII-only) with `@runtime_checkable class Adapter(Protocol)` (`name: str` annotation + `invoke(self, prompt: str, timeout: int) -> str`) and `class _SubprocessAdapterMixin` enforcing the full safe-subprocess contract via `_run_subprocess(argv, prompt, timeout)`: per-platform `subprocess.Popen` calls with `text=True, encoding="utf-8", errors="replace", shell=False`, list-form argv, prompt piped via stdin, mandatory timeout, `FileNotFoundError -> AdapterAuthError`, auth-marker substring match -> `AdapterAuthError`, empty-stdout-with-returncode-zero -> `AdapterError` naming `openai/codex#19945`, non-zero exit -> `AdapterError`, `subprocess.TimeoutExpired` -> `_kill_process_tree(proc)` (POSIX `os.killpg(os.getpgid(pid), SIGKILL)` after `start_new_session=True`; Windows `taskkill /T /F /PID` after `CREATE_NEW_PROCESS_GROUP`; the taskkill `subprocess.run` itself uses the full safe-contract kwargs so 04-03's TST-05 lint test will not flag it) -> `AdapterError`. Happy path returns `stdout.strip()`. All 5 plan-level verification commands PASS: imports, mypy --strict on `src/ultra_claude/exceptions.py src/ultra_claude/adapters/` clean, ruff clean, pytest 16/16 PASS (zero regression), LF-only on disk on all 3 files. Plus 6 inline integration smoke checks via `unittest.mock.patch('subprocess.Popen', ...)`: FileNotFoundError -> AdapterAuthError, empty-stdout+rc=0 -> AdapterError, auth-marker -> AdapterAuthError, happy path -> trimmed stdout, non-zero exit -> AdapterError, TimeoutExpired -> _kill_process_tree called + AdapterError. Four Rule-3 deviations all about strict-tooling acceptability without changing semantics: (1) per-platform Popen branches instead of dict-spread for mypy overload resolution; (2) `# type: ignore[attr-defined]` on two POSIX-only `os.getpgid`/`os.killpg` lines; (3) `contextlib.suppress(...)` instead of `try/except/pass` for ruff SIM105 (3 sites); (4) `# noqa: RUF022` + justifying comment for `__all__` chronological-by-introduction order. Atomic commits: `e4423d0` (feat: add AdapterError and AdapterAuthError exceptions) + `eceb9da` (feat: add Adapter Protocol and _SubprocessAdapterMixin). SUMMARY at `.planning/phases/04-adapter-protocol-claudeadapter/04-01-SUMMARY.md`. Plan 04-01 CLOSED; Phase 4 1/3 plans complete.

**Next action:** Plans 04-02 (ClaudeAdapter) and 04-03 (tests + TST-05 lint test) are now unblocked. 04-02 creates `src/ultra_claude/adapters/claude.py` with `ClaudeAdapter(_SubprocessAdapterMixin)` (`name="claude"`, `cli_name="claude"`, `auth_error_markers=("not logged in", "please run `claude login`", "authentication required")`, one-line `invoke` delegating to `_run_subprocess(["claude", "-p"], prompt, timeout)`) and updates `adapters/__init__.py` to re-export `ClaudeAdapter`. 04-03 creates `tests/test_adapters_base.py` (Protocol structural typing tests), `tests/test_adapter_claude.py` (5 paths via `pytest-subprocess` `fp` fixture: argv+stdin happy, empty stdout, FileNotFoundError, auth marker, TimeoutExpired+process-tree-kill), and `tests/test_subprocess_lint.py` (TST-05: `ast`-walks `src/ultra_claude/` and fails the build on any `subprocess.run`/`subprocess.Popen` missing `text=True`/`encoding="utf-8"`/`errors="replace"` or with `shell=True`). Phase 5 (Stop Conditions) remains parallelizable with the rest of Phase 4. Run `/gsd-execute-phase 4` to continue, or `/gsd-plan-phase 5` to decompose Phase 5.
...
- `src/ultra_claude/adapters/__init__.py` — newly landed in plan 04-01; re-exports `Adapter` + `_SubprocessAdapterMixin`; will gain `ClaudeAdapter` in 04-02 and `GeminiAdapter`/`CodexAdapter` in Phase 7
- `src/ultra_claude/adapters/base.py` — newly landed in plan 04-01; the choke point for every ultra-claude adapter (Adapter Protocol + safe-subprocess mixin); consumed by 04-02 (ClaudeAdapter), 04-03 (tests + TST-05 lint), Phase 7 (Gemini/Codex adapters)
- `.planning/phases/04-adapter-protocol-claudeadapter/04-CONTEXT.md` — phase 4 context (decisions, code insights, specifics)
- `.planning/phases/04-adapter-protocol-claudeadapter/04-01-PLAN.md` — completed plan (this plan)
- `.planning/phases/04-adapter-protocol-claudeadapter/04-01-SUMMARY.md` — completion summary for 04-01 (this plan)
- `.planning/phases/04-adapter-protocol-claudeadapter/04-02-PLAN.md` — pending plan (ClaudeAdapter)

...
- [x] **PKG-01
**: User can `pip install ultra-claude` from PyPI and the `ultra-claude` command is on PATH
...
- [x] **TST-01
**: Test suite passes via `pytest` without any of `claude`/`gemini`/`codex` installed (subprocess fully mocked)
- [x] **TST-02
**: Adapter tests use `pytest-subprocess`'s `fp` fixture to assert exact argv, stdin payload, and timeout
...
- [x] **TST-06
**: `ruff check` and `mypy` (configured strict for `src/ultra_claude/`) pass with zero errors
- [x] **TST-07
**: Manual `python -m build` produces a wheel and sdist that install cleanly into a fresh virtualenv on macOS, Linux, and Windows (smoke-tested before tagging v0.1.0)
...
|-------------|-------|--------|
| PKG-01 | Phase 9 | Autonomous half complete (2026-05-02, plan 09-04 -- `python -m build` produced `dist/ultra_claude-0.1.0-py3-none-any.whl` 40407 bytes + `dist/ultra_claude-0.1.0.tar.gz` 33618 bytes; both PASS `twine check`; smoke install in fresh venv on Windows 11 / Python 3.11.9 PASSED -- `ultra-claude --version` -> `0.1.0`; `ultra-claude run --preset debate --inline "test" --dry-run` -> exit 0 with 9-turn schedule). User-action half (`pip install ultra-claude` from PyPI on a fresh machine) closes after the user runs `python -m twine upload dist/ultra_claude-0.1.0*` per PUBLISH.md runbook |
...
| PKG-06 | Phase 9 | Autonomous half complete (2026-05-02, plan 09-01 commits `8ade3e6` + `bc8e3d1` + `6155dc6` for 0.1.0 literal + py.typed marker + CHANGELOG [0.1.0] section; plan 09-04 commits `e462b8f` + `1e4d2f3` for the build+smoke+quality-gates+PUBLISH.md update). User-action half: run `python -m twine upload dist/ultra_claude-0.1.0*` per PUBLISH.md runbook to publish to PyPI; this single upload closes both PKG-05 (Phase 1's never-uploaded 0.0.1 stub is superseded; v0.1.0 claims the name) and PKG-06 (the v0.1.0 functional release) |
...
| TST-01 | Phase 9 | Complete (2026-05-02, plan 09-04 verified -- `pytest --cov=src/ultra_claude --cov-report=term-missing` runs in the dev venv with NONE of `claude`/`gemini`/`codex` real CLIs touched: 86/86 PASS in 4.77s; adapter tests use `pytest-subprocess`'s `fp` fixture (Phase 4 + Phase 7); orchestrator E2E test (`tests/test_e2e_with_echo_cli.py` from 09-02) uses `tests/fixtures/echo_cli.py` as a fake CLI; the smoke install test in `.smoke-venv-09` confirms a fresh venv with zero dev deps installs the wheel cleanly and runs `--dry-run` without any agent CLIs) |
| TST-02 | Phase 9 | Complete (2026-05-02, plan 09-04 verified -- pytest-subprocess argv/stdin/timeout assertions are exercised across `tests/test_adapter_claude.py` + `tests/test_adapter_gemini.py` + `tests/test_adapter_codex.py` (Phase 4 + Phase 7) which use `fp.calls` to assert exact argv shape (`["claude", "-p"]` / `["gemini", "-p"]` / `["codex", "exec"]`), pipe stdin payload via `fp.stdin`, and `timeout=` keyword propagation. All three adapter test files inherit the safe-contract assertions from the shared fixture pattern. Confirmed working in plan 09-04's full-suite run: 86/86 PASS) |
...
| TST-06 | Phase 9 | Complete (2026-05-02, plan 09-04 commit `e462b8f` -- the 4 pre-existing Phase 2 ruff errors (`config.py` RUF022 + UP037 / `tests/test_config.py` I001 + F401) cleared as Rule 3 deviation; `ruff check` reports "All checks passed!" project-wide; `mypy` reports "Success: no issues found in 13 source files" against `src/ultra_claude/` with strict=true; coverage gate satisfied -- `pytest --cov=src/ultra_claude` reports TOTAL line coverage 85% (>80% bar)) |
| TST-07 | Phase 9 | Autonomous half complete (2026-05-02, plan 09-04 -- clean-venv smoke install on Windows 11 / Python 3.11.9: `python -m venv .smoke-venv-09 && .smoke-venv-09/Scripts/python.exe -m pip install --no-cache-dir dist/ultra_claude-0.1.0-py3-none-any.whl` resolved 8 transitive deps cleanly; `ultra-claude --version` -> `ultra-claude, version 0.1.0`; `ultra-claude run --preset debate --inline "test" --dry-run` -> exit 0 with 9-turn schedule containing all 3 agents on claude/gemini/codex). Cross-platform CI smoke on macOS + Linux deferred to post-v0.1.0 GitHub Actions (deferred per locked decision; out of scope for v1 release) |

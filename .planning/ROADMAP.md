...
- [x] 08-02-PLAN.md — src/ultra_claude/cli.py click group with run + doctor subcommands, --version, all flags, exit-code mapping, TTY-aware logging (CLI-01..CLI-11) — COMPLETE 2026-05-02 (commit f452152); CLI-01..CLI-11 marked complete in REQUIREMENTS.md (11 requirements); src/ultra_claude/cli.py created (405 lines / 15458 bytes / LF-only / ASCII-only) implementing click group `main` + `run` subcommand (TASK_FILE positional + 6 flags: --config, --preset, --inline, --dry-run, --output, --abort-on-error) + `doctor` subcommand; ConfigError -> ctx.exit(2), AdapterError (incl. AdapterAuthError subclass) -> ctx.exit(1) per CLI-10 mapping; TTY-aware logging via `_configure_logging` (BOTH stdout AND stderr ttys -> INFO; otherwise WARNING) per CLI-11; preset loading via `importlib.resources.files("ultra_claude.presets").joinpath("<name>.yaml")` per CLI-05/PRE-01; doctor probes via shutil.which + subprocess.run with FULL safe-contract kwargs (text=True, encoding="utf-8", errors="replace", shell=False, timeout=5s, list-form argv, input="") per CLAUDE.md Critical Constraint #1; programmatic --version invocation works (`python -c "from ultra_claude.cli import main; main(['--version'], standalone_mode=False)"` prints "ultra-claude, version 0.0.1"); 72/72 full suite PASS (zero regression — plan adds zero Python test code; that lands in 08-03); mypy --strict on src/ultra_claude clean (13 source files = 12 prior + cli.py); ruff check on cli.py clean; TST-05 lint test 3/3 PASS (single subprocess.run site in cli.py has all required kwargs); all 5 manual smoke checks PASS (--version, --help with `run` + `doctor` listed, --preset debate dry-run rendering 9-turn round-robin plan, missing-config -> exit 2, doctor printing 4-column ASCII table); zero Rule-N deviations (plan executed verbatim with one trivial cosmetic line-break in the `any()` argument tuple for ruff line-length compliance); zero deletions; one environment-specific finding logged to deferred-items.md (Windows .cmd shim limitation in `subprocess.Popen([...], shell=False)`; affects production adapter codepath equally; NOT a regression; v2 mitigation candidate)
- [x] 08-03-PLAN.md — tests/test_cli.py with 11 CliRunner-based tests covering CLI-01..CLI-11 + PRE-01 (zero real subprocess launches) — COMPLETE 2026-05-02 (commit `4ada905`); 11 new tests cover --version, --help, --inline+--dry-run, --preset debate, --config <path>, dry-run turn order, doctor table, ConfigError -> exit 2, AdapterError + --abort-on-error -> exit 1, end-to-end pipeline via FakeAdapters, stdout-only discipline; 83/83 full suite PASS in 1.17s (72 prior + 11 new; zero regression); TST-05 lint 3/3 PASS; ruff clean on tests/test_cli.py (1 I001 import-organization auto-fix applied during execution); mypy --strict on src/ultra_claude clean (13 source files; zero source changes); LF-only + ASCII-only on disk (15515 bytes / 0 CRLF / 457 LF / 0 non-ASCII); staged blob LF-only despite `core.autocrlf=true`; FakeAdapter mirrored from tests/test_orchestrator.py to keep this test file independently runnable in isolation; end-to-end test injects FakeAdapters via `monkeypatch.setattr(ultra_claude.orchestrator, "get_adapter", fake_factory)` (the orchestrator's module-level binding seam); doctor probe test mocks `cli_module.shutil.which` + `cli_module.subprocess.run` and the fake_run callable additionally asserts safe-contract kwargs (text=True / encoding="utf-8" / errors="replace" / shell=False / timeout) on every call (defense-in-depth alongside TST-05 lint); 2 Rule-N deviations (Rule 3: `CliRunner(mix_stderr=False)` removed in click 8.3.3 — replaced with default-split `CliRunner()`; Rule 3: ruff I001 import-organization auto-fix); CLI-01..CLI-11 + PRE-01 all both IMPLEMENTATION (08-02) AND TEST (08-03) verified; **Phase 8 fully CLOSED (3/3 plans, 12/12 requirements)**; Phase 9 (release prep) UNBLOCKED
**UI hint**: no
...
| 8. CLI Surface & `debate` Preset | 3/3 | COMPLETE — Plan 08-01 (commits `481c8e9` + `7331fc7`, pyproject `[project.scripts]` entry point + bundled debate.yaml preset; PRE-01) + Plan 08-02 (commit `f452152`, src/ultra_claude/cli.py click group with run + doctor + 6 flags + exit-code mapping + TTY-aware logging; CLI-01..CLI-11) + Plan 08-03 (commit `4ada905`, tests/test_cli.py with 11 CliRunner-based tests covering CLI-01..CLI-11 + PRE-01); CLI-01..CLI-11 + PRE-01 all both IMPLEMENTATION + TEST verified; 83/83 full suite PASS (72 prior + 11 new tests = 83; zero regression); mypy --strict on src/ultra_claude clean (13 source files); ruff clean on all source + new test files; TST-05 lint 3/3 PASS; LF-only + ASCII-only on disk on all new files | 2026-05-02 |
| 9. Tests, Docs, Examples & v0.1.0 Release | 3/4 | IN PROGRESS — Plan 09-01 (commits `8ade3e6` + `bc8e3d1` + `6155dc6`, version bump 0.0.1->0.1.0 + py.typed PEP 561 marker + CHANGELOG.md [0.1.0] section; PKG-06 implementation half) + Plan 09-02 (commits `58ec2f8` + `2575869`, tests/fixtures/echo_cli.py fake-CLI fixture + tests/test_e2e_with_echo_cli.py 3-test E2E suite using real subprocess.Popen via _SubprocessAdapterMixin; TST-03 + TST-04 closed) + Plan 09-03 (commits `078dc7c` + `180be45` + `2ab93b7`, README.md rewrite from 12-line stub to 130-line / 6313 bytes full v0.1.0 README + CONTRIBUTING.md 92 lines / 5608 bytes + examples/ tree (4 files: README.md / debate.yaml byte-identical to bundled preset / sample-debate.md with TRX-02 sentinels / sample-debate.md.jsonl with 3 TurnRecord-shaped JSON records); DOC-01 + DOC-02 + PRE-02 closed); 86/86 full suite PASS (zero regression across all 3 plans); LF-only on disk + LF-only in staged blobs despite `core.autocrlf=true` host on every artefact; ASCII-only on disk on every plan artefact; only Plan 09-04 remaining (python -m build wheel+sdist + clean-venv smoke install + pytest --cov + ruff + mypy gates + PUBLISH.md v0.1.0 section) | - (closes when 09-04 completes + user `twine upload`) |
...
*Last updated: 2026-05-02 after plan 08-03 autonomous completion (**Phase 8 COMPLETE — 3/3 plans**; tests/test_cli.py landed via commit 4ada905 with 11 CliRunner-based tests covering CLI-01..CLI-11 + PRE-01; 83/83 full suite PASS — 72 prior + 11 new = 83; zero regression; FakeAdapter mirrored from tests/test_orchestrator.py for runtime isolation; end-to-end test injects FakeAdapters via `monkeypatch.setattr(ultra_claude.orchestrator, "get_adapter", fake_factory)` — the orchestrator's module-level binding seam; doctor probe test mocks `cli_module.shutil.which` + `cli_module.subprocess.run` and the fake_run callable additionally asserts safe-contract kwargs on every call (defense-in-depth alongside TST-05 lint); 2 Rule-N deviations (Rule 3: `CliRunner(mix_stderr=False)` parameter removed in click 8.3.3 — replaced with default-split `CliRunner()`; Rule 3: ruff I001 import-organization auto-fix); CLI-01..CLI-11 + PRE-01 all both IMPLEMENTATION (08-02) AND TEST (08-03) verified; zero source changes — test-only plan; Phase 9 (Tests, Docs, Examples & v0.1.0 Release) UNBLOCKED)*
...
*Plan 08-01 completed: 2026-05-02 (commits 481c8e9 — feat(08-01): add ultra-claude console-script entry point + 7331fc7 — feat(08-01): add bundled debate preset (PRE-01)); PRE-01 marked complete in REQUIREMENTS.md; pyproject.toml gains `[project.scripts]` ultra-claude = "ultra_claude.cli:main"; src/ultra_claude/presets/debate.yaml created (1033 bytes / 30 lines / LF-only / ASCII-only) bundling 3-agent roundtable (Architect: claude, Critic: gemini, Implementer: codex) with max_turns=9, stop_keywords=[AGREED, SHIP IT]; reachable from `importlib.resources.files('ultra_claude.presets')`; 72/72 full suite PASS (zero regression); zero Rule-N deviations*
*Plan 08-02 completed: 2026-05-02 (commit f452152 — feat(08-02): add cli.py click group with run + doctor (CLI-01..CLI-11)); CLI-01..CLI-11 marked complete in REQUIREMENTS.md (11 requirements); src/ultra_claude/cli.py created (405 lines / 15458 bytes) implementing click group `main` + `run` subcommand (TASK_FILE positional + 6 flags: --config, --preset, --inline, --dry-run, --output, --abort-on-error) + `doctor` subcommand; ConfigError -> ctx.exit(2), AdapterError -> ctx.exit(1) per CLI-10 mapping; TTY-aware logging per CLI-11; preset loading via importlib.resources per CLI-05/PRE-01; doctor probes via shutil.which + subprocess.run with FULL safe-contract kwargs per CLAUDE.md Critical Constraint #1; programmatic --version invocation works; 72/72 full suite PASS (zero regression); mypy --strict on src/ultra_claude clean (13 source files); ruff clean; TST-05 lint 3/3 PASS; all 5 manual smoke checks PASS; zero Rule-N deviations; one environment-specific finding logged to deferred-items.md (Windows .cmd shim limitation)*
*Plan 08-03 completed: 2026-05-02 (commit 4ada905 — test(08-03): add CLI test suite via click CliRunner (CLI-01..CLI-11 + PRE-01)); 11 new tests across tests/test_cli.py (457 lines / 15515 bytes / LF-only / ASCII-only / UTF-8) cover --version (CLI-01), --help with subcommands listed (CLI-02), --inline + --dry-run (CLI-06+CLI-07), --preset debate without local YAML (CLI-05+PRE-01), --config <path> override (CLI-04), full turn order in dry-run output (CLI-07), doctor 4-column ASCII status table with safe-contract kwargs assertion (CLI-09), ConfigError -> exit 2 (CLI-10), AdapterError + --abort-on-error -> exit 1 (CLI-10), end-to-end pipeline producing real transcript via FakeAdapter injection (CLI-03+CLI-08), and stdout-only-contains-transcript-path discipline (CLI-11); FakeAdapter mirrored from tests/test_orchestrator.py for runtime isolation; end-to-end test injects FakeAdapters via `monkeypatch.setattr(ultra_claude.orchestrator, "get_adapter", fake_factory)` — the orchestrator imports the symbol at module top level so this seam replaces the binding the run() loop uses; doctor probe test mocks cli_module.shutil.which + cli_module.subprocess.run; fake_run additionally asserts safe-contract kwargs (text=True / encoding="utf-8" / errors="replace" / shell=False / timeout) on every call (defense-in-depth alongside TST-05 lint); zero real subprocess launches in the test file; 83/83 full suite PASS in 1.17s (72 prior + 11 new; zero regression); TST-05 lint 3/3 PASS; ruff clean (1 I001 import-organization auto-fix applied during execution); mypy --strict on src/ultra_claude clean (13 source files; zero source changes); LF-only + ASCII-only on disk (15515 bytes / 0 CRLF / 457 LF / 0 non-ASCII); staged blob LF-only despite `core.autocrlf=true`; 2 Rule-N deviations (Rule 3: `CliRunner(mix_stderr=False)` parameter removed in click 8.3.3 — replaced with default-split CliRunner(); Rule 3: ruff I001 import-organization auto-fix); CLI-01..CLI-11 + PRE-01 all both IMPLEMENTATION (08-02) AND TEST (08-03) verified; zero source changes; **Phase 8 fully CLOSED (3/3 plans, 12/12 requirements)**; Phase 9 (Tests, Docs, Examples & v0.1.0 Release) UNBLOCKED*
